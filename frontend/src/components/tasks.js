...
function Tasks() {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:3001/getTasksList')
            .then(result => {
                settaskManagement(result.data)
            })
    ...
    const toggleEditable = (id) => {
        const rowData = taskManagement.find((data) => data._id === id);
        if (rowData) {
            setEditableId(id);
    ...
    const addTask = (e) => {
        ...
        axios.post('http://127.0.0.1:3001/addTasksList', 
            { 
                task: newTask, 
                status: newStatus, 
            ...
            .then(res => {
                console.log(res);
                settaskManagement([...taskManagement, res.data]);
                setNewTask("");
                setNewStatus("");
                setNewDeadline("");
            })
    ...
    const saveEditedTask = (id) => {
        ...
        axios.post('http://127.0.0.1:3001/updateTasksList/' + id, editedData)
            .then(result => {
                console.log(result);
                setEditableId(null);
                setEditedTask("");
    ...
    const deleteTask = (id) => {
        axios.delete('http://127.0.0.1:3001/deleteTasksList/' + id)
            .then(result => {
                console.log(result);
                settaskManagement(taskManagement.filter(task => task._id !== id));
            })
            .catch(err =>
                console.log(err)
            )
                ...
                <div className="col-md-7">
                    ...
                    <div className="table-responsive">
                        <table className="table table-bordered">
                            ...
                            {Array.isArray(taskManagement) ? (
                                <tbody>
                                    {taskManagement.map((data) => (
                                        <tr key={data._id}>
                                            <td>
                                                {editableId === data._id ? (
                                                    <input
